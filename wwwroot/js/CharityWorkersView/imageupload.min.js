...
(function($) {
    ...
    function getImageThumbnailHtml(src) {
        return '<div class="img-holder"><img src="' + src + '" alt="Image preview" class="thumbnail" style="max-width: ' + options.maxWidth + 'px; max-height: ' + options.maxHeight + 'px"></div>';
    }
    ...
    function isValidImageFile(file, callback) {
        ...
        if ($.inArray(fileExtension, options.allowedFormats) > -1) {
            ...
            callback(false, 'امتداد الصوره غير مدعم.');
        }
    }
    ...
    function resetFileTab($fileTab) {
        ...
        $fileTab.find('.img-holder').remove();
        $fileTab.find('.btn span').html('<i class="fa fa-folder-open-o"></i>');
    ...
    function submitImageFile($fileTab) {
        ...
        $fileTab.find('.alert').remove();
        $fileTab.find('.img-holder').remove();
        $browseFileButton.find('span').html('<i class="fa fa-folder-open-o"></i>');
